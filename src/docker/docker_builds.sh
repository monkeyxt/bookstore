...
# This image should be run with "docker run order NAME=[name]" where name is order1, order2, etc
echo "Building order image..."
docker build .. -f docker/order.Dockerfile -t order

# This image should be run with "docker run catalog NAME=[name]" where name is catalog1, catalog2, etc
